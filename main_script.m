% Main file to run the copepod model.
%Several scenarios can be done from this file:
        %The steady scenarios (environment is constant over time)--> select "seasoanl_switch=0" below
        %The seasonal environment --> select "seasoanl_switch=1" below
        %Parameter switch--> select "sensit_switch=1" below
        %Right now it is set for the parameter "rho" only.
        %But can be switched in file "function_call_sensitivity_analysis_diags.m"
        
%parameter values, sizes of copepods and protists etc can be modified in
%the file "parameters_copepod_model.m".
        
% A code that is more user friendly will be uploaded soon.

% Camila Serra-Pompei 12/02/2020
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clearvars
...
%for both constant and seasonal environment:
yearsrun=100; %for how many years to we want the run
N_input_rate=10^-2; %parameter of input of nutrients to the system (only works for constan environment, seasonal is defined in "func_seasonal_forcing")
...
sensit_switch=0; %set to 0 for a simple run, set to 1 to run the parameter sweep (seasonal_switch must be =0)
if sensit_switch==1
   seasonal_switch=0; 
end
...
max_val=-1; %max values of parameter sweep.
