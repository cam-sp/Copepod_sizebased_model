function [theta_P_P,theta_P_D, theta_P, theta_cop_P, theta_cop_cop, theta_cop_D, theta_cop]=func_feeding_kernels(param)
...
% theta_cop_cop(param.ind_j,param.ind_a) = theta_cop_cop(param.ind_j,param.ind_a)./(log(param.C_dw(param.ind_a))-log(param.C_up(param.ind_a)));

% %correct for the adult izes
% theta_cop_cop(param.ind_a,:) = theta_cop_cop(param.ind_a,:)./(-param.Wvec(param.ind_a-1));

% mprey=logspace(-5,1,1000);
% mpred=100;
% pred_adults=exp(-((log(param.beta_act*mprey./mpred)).^2)/(2*param.sigma_act^2));
% figure
% semilogx(mprey,pred_adults)
...
% theta_cop_cop(param.ind_a,i)=exp(-((log(beta_c*param.W(i)./param.W(param.ind_a))).^2)./(2*sigma_c^2));
theta_cop_cop(i,param.ind_a)=exp(-((log(beta_c*param.W(param.ind_a)./param.W(i))).^2)./(2*sigma_c^2));

% theta_cop_cop2(:,i)=exp(-((log(beta_c*param.W(i)./param.W(:))).^2)./(2*sigma_c^2));
% theta_cop_cop2(i,:)=exp(-((log(beta_c*param.W(param.ind_a)./param.W(i))).^2)./(2*sigma_c^2));
...
theta_cop_D=zeros(length(param.W(:)),param.nbr_D);
...
theta_cop_D(j,:) = sqrt(pi/2)*sigma_c* (...
  erf( (log(param.D_dw)-log(pred_vec(j)/beta_c))/(sqrt(2)*sigma_c) ) ...
  - erf( (log(param.D_up)-log(pred_vec(j)/beta_c))/(sqrt(2)*sigma_c) ));

end
theta_cop_D=theta_cop_D./(log(param.D_dw)-log(param.D_up));
if param.C_sp_pass==0
theta_cop_D(:)=0; %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
end
...
% theta_cop_P(:,end-1:end)=0;

% theta_cop_cop(:)=0;

theta_cop=[theta_cop_P, theta_cop_cop, theta_cop_D];
