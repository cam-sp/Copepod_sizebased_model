function [dxdt,Diagnostics_vec,Diagnostics_vec_protists,reproduction,detrit_flux,massbalance,pred_C_on_D,FPP_tot] =function_model_NPZF(t,x,param,theta_P,theta_cop,theta_cop_P,theta_cop_cop,theta_cop_D,Ls,zmld2,...
    dzdt2,Diff2,temp,seasonal_switch,diagnostics_switch,sensit_switch)
...
mu_max=param.mu_max.*param.Q_k.^((T-18)/10);
...
if seasonal_switch==1 %dormancy
    dorm_idx=find(nu<0);
    nu(dorm_idx)=param.eff(dorm_idx).*I(dorm_idx).*(F_lvl(dorm_idx)-fc(dorm_idx)./4); %switch to dormancy
%     if L<5
...
elseif seasonal_switch==1
    dNdt=dNdt+ (sum(k(dorm_idx)/4.*C(dorm_idx)))./param.Qcn; %careful here!!!!! it is by 4 now
end    
