%Here you can find figures related to the seasonal scenario

clearvars

%load seasonal scenario workspace
load('workspace_seasonal.mat')
...
FPPtot=sum(FPP_prod_diag,2);

axes(ha(7))
yyaxis left
plot(t(end-365*yr:end-yrend),fluxmld(end-365*yr:end-yrend)./zmld(end-365*yr:end-yrend),'k','linewidth',1.5) %./zmld(end-365*yr:end-yrend)
hold on
plot(t(end-365*yr:end-yrend),flux1000(end-365*yr:end-yrend)./zmld(end-365*yr:end-yrend),'k--','linewidth',1.5)
ylabel({'Fecal pellets flux';'[mg m^{-2} d^{-2}]'})
% ylim([0 1.5])
set(gca,'YTick',[0 0.5 1]);
% semilogy(t(end-365*yr:end-yrend),N(end-365*yr:end-yrend),'k:')
yyaxis right
% plot(t(end-365*yr:end-yrend),fluxmld(end-365*yr:end-yrend)./FPPtot(end-365*yr:end-yrend),'color',cp,'linewidth',1.5)
% hold on
...
%% Plots seasonality several years
